 ...
 * **warningClass**: it's the class of the element with the indicator. By default is the bootstrap "label badge-success" but can be changed to anything you'd like.
 * **limitReachedClass**: it's the class the element gets when the limit is reached. Default is "label label-important badge-danger" (to support Bootstrap 2 and 3).
...
$('input.className').maxlength({
    alwaysShow: true,
    threshold: 10,
    warningClass: "label badge-info",
    limitReachedClass: "label badge-warning",
    placement: 'top',
    preText: 'used ',
    separator: ' of ',
    postText: ' chars.'
